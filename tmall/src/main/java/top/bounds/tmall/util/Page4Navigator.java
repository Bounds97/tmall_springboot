...
/**
 * @author Bounds
 * @param <T> 分页类，封装了JPA 默认提供了一个分页类：org.springframework.data.domain.Page
 */
public class Page4Navigator<T> {
	// jpa 传递出来的分页对象， Page4Navigator 类就是对它进行封装以达到扩展的效果
	Page<T> pageFromJPA;

	// 分页的时候 ,如果总页数比较多，那么显示出来的分页超链一个有几个。 比如如果分页出来的超链是这样的： [8,9,10,11,12], 那么
	// navigatePages 就是5
	int navigatePages;
	...
	// 分页的时候 ,如果总页数比较多，那么显示出来的分页超链一个有几个。 比如如果分页出来的超链是这样的： [8,9,10,11,12]，那么
	// navigatepageNums 就是这个数组：[8,9,10,11,12]，这样便于前端展示
